# ------------------------------------------------------------------------------------
# CLIP-VQGAN
# Copyright (c) 2023 Shan Yang. All Rights Reserved.
# Licensed under the MIT License [see LICENSE for details]
# ------------------------------------------------------------------------------------
# Modified from Enhancing Transformers
...
class ViTEncoder(nn.Module):
    def __init__(self, clip_name: str, image_size:  int, patch_size: int, channels: int = 3) -> None:
        super().__init__()
        ...
        assert self.image_height % self.patch_size == 0 and self.image_width % self.patch_size == 0, 'Image dimensions must be divisible by the patch size.'
        
        self.num_patches = (self.image_height // patch_size) * (self.image_width // patch_size)
        self.patch_dim = channels * patch_size * patch_size
        
        self.padding=Pad(((self.image_height-self.patch_size)//2,(self.image_height-self.patch_size)//2),fill=0,padding_mode='constant')
        self.clip, _ = clip.load(clip_name)
        self.clip.eval()
    ...
    def forward(self, img: torch.FloatTensor) -> torch.FloatTensor:
        ...
        patches=patches.contiguous().view(-1,self.channels,self.patch_size,self.patch_size)

        x=self.clip(patches).view(batch_size,-1,self.clip.visual.output_dim)        
        return x
